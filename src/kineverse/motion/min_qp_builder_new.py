...
from kineverse.gradients.diff_logic         import create_symbol,   \
                                                   get_symbol_type, \
                                                   get_int_symbol,  \
                                                   erase_type,      \
                                                   Symbol,          \
...
def rich_diff(s, expr):
    ts = get_symbol_type(s)
    bs = erase_type(s)

    for x in range(TYPE_POSITION, TYPE_SNAP + 1):
        a = create_symbol(bs, x)
